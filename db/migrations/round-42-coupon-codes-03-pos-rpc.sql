-- round-42-coupon-codes-03-pos-rpc.sql
-- Add coupon support to confirm_pos_sale.
--
...
--     (server-side, channel 'pos', base = merchandise subtotal) -- it never
--     trusts a client-supplied coupon amount. An invalid/expired code raises.
--   * sales.discount_cents = manual + coupon (clamped so it can't exceed the
--     subtotal; the coupon portion is reduced first if needed).
