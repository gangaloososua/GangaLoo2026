-- round-42-coupon-codes-05-storefront-rpc.sql
-- Add coupon support to place_storefront_order (public customer checkout).
--
-- WHAT CHANGED (everything else is byte-for-byte the original):
--   * Reads optional payload->>'coupon_code'.
--   * After the line loop (subtotal known), validates the code server-side via
--     validate_coupon (channel 'online', store = the storefront warehouse,
--     base = merchandise subtotal). A bad/expired/wrong-scope code is IGNORED
--     (zero discount) — it never fails the order; the reason is returned so the
--     page can explain. This also avoids a public code-enumeration endpoint.
--   * Sets sales.discount_cents to the coupon amount (clamped to subtotal) and,
--     when applied, writes a sale-level audit row (is_manual=false, rule_id).
--   * total_cents (derived) then reflects the discount; returned in the quote.
--   * confirm_storefront_order is UNCHANGED — it already reads discount_cents
--     off the draft and folds it into gross profit.
