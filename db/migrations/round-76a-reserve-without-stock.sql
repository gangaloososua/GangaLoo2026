-- round-76a-reserve-without-stock.sql
-- Allow reserving brand-new products that have NO inventory lot in the source
-- warehouse.
--
-- BACKGROUND: confirm_pos_sale() already allowed overselling into NEGATIVE
-- stock for SOLD-OUT products (a lot exists at qty_remaining 0; the shortfall
-- is hung on the first lot and that lot goes negative). The ONLY hard block was
-- brand-new products with zero lot rows in the warehouse: there was no lot to
-- attach the shortfall to, so it raised 'no_lots_for_product' and refused the
-- reservation.
--
-- FIX: when a product has no lot in the warehouse, auto-create a zero-quantity
-- placeholder lot (qty_received 0, qty_remaining 0, unit_cost_dop 0,
-- lot_number 'RESERVE-AUTO') instead of raising. The existing negative-stock
-- logic then drives that lot's qty_remaining negative (e.g. -1), which IS the
-- "needs ordering" signal surfaced in the Inventory Reserved view. COGS stays 0
-- until real stock is received at a real cost, at which point gross profit
-- trues up. Sold-out path, ledger, invoice, commissions, discounts: UNCHANGED.
--
-- Rebuilt from the LIVE body via pg_get_functiondef; the only delta is the
-- RAISE EXCEPTION 'no_lots_for_product...' block replaced by the auto-lot
-- INSERT. Applied live in Supabase; this file is the record.
...
BEGIN
  ...
  IF v_user_role IS NULL
     OR v_user_role NOT IN ('owner','admin','seller','distributor') THEN
    RAISE EXCEPTION 'permission denied: only owner/admin/seller/distributor can ring up POS sales'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF v_coupon_code IS NOT NULL THEN
    SELECT vc.rule_id, vc.discount_cents, vc.delta_percent, vc.reason
      ...
      FROM public.validate_coupon(
             ...
             'pos'::public.sale_source, v_subtotal_cents, now()
           ) vc;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items) LOOP
    ...
    IF v_is_inventory THEN
      ...
      -- Round 76: auto-create a zero-qty placeholder lot for brand-new products
      -- with no lot in this warehouse, instead of raising no_lots_for_product.
      IF v_first_lot_id IS NULL THEN
        INSERT INTO inventory_lots (
          product_id, warehouse_id, lot_number,
          qty_received, qty_remaining, unit_cost_dop
        ) VALUES (
          v_product_id, v_source_warehouse_id, 'RESERVE-AUTO',
          0, 0, 0
        )
        RETURNING id, unit_cost_dop INTO v_first_lot_id, v_first_lot_unit_cost;
      END IF;
