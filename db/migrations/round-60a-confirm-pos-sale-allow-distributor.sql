-- round-60a-confirm-pos-sale-allow-distributor.sql
--
-- Lets DISTRIBUTORS ring up POS sales (the register), alongside
-- owner / admin / seller. Distributors could already open /caja and
-- build a cart, but confirm_pos_sale rejected them at submit with
-- "only owner/admin/seller can ring up POS sales".
--
-- ONLY CHANGE vs round-39-noninventory-products-02-confirm-pos-sale.sql:
--   * the role gate now allows 'distributor' too
--   * the error message text updated to match
-- Everything else (stock/FIFO/COGS, commissions, ledger posting,
-- paid-status logic) is byte-for-byte identical to round-39.
--
-- Safe to re-run: it is a CREATE OR REPLACE of one function.
