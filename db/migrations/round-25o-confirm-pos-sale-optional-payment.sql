-- round-25o-confirm-pos-sale-optional-payment.sql
-- Makes payment OPTIONAL in confirm_pos_sale. With zero payments the sale is
-- created as a 'confirmed' (unpaid) ORDER: stock is consumed, commission is
-- pending, and NOTHING posts to the ledger (no payment rows -> the posting
-- loop does nothing). Sellers/distributors create orders this way at the POS;
-- the owner records the real payment later. Only changes vs round-24e:
--   1. removed the "at least one payment is required" guard, and
--   2. the 'paid' status now also requires v_paid_cents > 0 (so a zero-total,
--      zero-paid order can't be mislabelled 'paid').
-- Everything else is byte-for-byte the prior confirm_pos_sale.
-- Verified reversibly: a no-payment order returns status=confirmed,
-- paid_cents=0, ledger_txns=0.
...
BEGIN
  ...
  END IF;
  -- (Round 25o) Payment is now OPTIONAL: zero payments => an unpaid 'confirmed'
  -- order. Sellers/distributors create orders this way; the owner records the
  -- real payment later. With no payment rows, nothing posts to the ledger.
