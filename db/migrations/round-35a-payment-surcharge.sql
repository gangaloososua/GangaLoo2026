-- round-35a-payment-surcharge.sql
-- Groundwork for online card payments (Stripe / PayPal). NO real charging yet --
-- this only adds the surcharge plumbing + option gating so we can build and test
-- the money math before wiring the actual payment SDKs.
--
-- Design choices:
--   * The surcharge is stored in a NEW column sales.payment_fee_cents (default 0),
--     NOT folded into the generated total_cents column (other screens/views depend
--     on total_cents, so we leave it untouched). The amount a card customer pays
--     is total_cents + payment_fee_cents.
--   * Surcharge % + fixed come from existing store_config keys:
--       Stripe : stripe_fee_pct (6), stripe_fee_fixed (0)
--       PayPal : extra_pay_fee_pct (6), extra_pay_fee_fixed (0), extra_pay_name
--     "fixed" is interpreted in PESOS (consistent with the other money settings).
--   * A new flag online_pay_enabled (default 0/off) gates whether the storefront
--     shows the Stripe/PayPal options at all. Stays OFF until real charging is
--     built + tested, so customers never see a non-working button.
--   * place_storefront_order now accepts payment_method 'stripe' | 'paypal' too,
--     computes the surcharge server-side, and stores it. Orders are still created
--     as unpaid 'draft' (charging comes in a later step).

alter table sales add column if not exists payment_fee_cents integer not null default 0;
comment on column sales.payment_fee_cents is
  'Online card surcharge (Stripe/PayPal). Amount a card customer pays = total_cents + payment_fee_cents.';

-- Off by default; owner flips to 1 to expose the card options once charging works.
insert into store_config (key, value, description) values
  ('online_pay_enabled', to_jsonb(0), 'Show Stripe/PayPal options on storefront checkout (1=yes,0=no)')
on conflict (key) do nothing;

-- Public storefront config now also returns the payment-fee settings + the flag,
-- so checkout can preview the surcharge. (Still SECURITY DEFINER, narrow.)
...
-- place_storefront_order: accept stripe/paypal, compute + store the surcharge.
...
begin
  ...
  loop
    ...
    v_unit_deal := round(v_normal * (1 - v_deal_frac))::int;
