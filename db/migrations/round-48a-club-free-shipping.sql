-- round-48a-club-free-shipping.sql
-- Club members get FREE shipping online. Re-creates place_storefront_order
-- (from round-46b) with one added rule: when the buyer is a Club member, the
-- shipping charge is set to 0. The checkout DISPLAY is updated in lockstep
-- (checkout-view.tsx + getOrderQuote) so the on-screen total and the charge
-- always agree. Non-members are unchanged.
--
-- get_storefront_quote is unchanged; it already reports is_club_member, which
-- the checkout page uses to show shipping as "Gratis" for members.
...
begin
  ...
  -- Club members get free shipping.
  if v_is_member then
    v_ship := 0;
  end if;
