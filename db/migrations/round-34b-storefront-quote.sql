-- round-34b-storefront-quote.sql
-- Step B (checkout display) support for loyalty tiers.
--
-- 1. get_storefront_quote(payload): READ-ONLY. Prices a cart exactly like
--    place_storefront_order (normal price -> deal -> tier, combined cap 30%),
--    for the LOGGED-IN customer (auth.uid()). Returns the subtotal before the
--    member discount, the member discount amount, and the tier name/percent so
--    checkout can show an accurate "Member discount (Bronze 5%)" line. Creates
--    nothing. Guests (not logged in) get tier 0 -> no member discount preview.
--
-- 2. place_storefront_order: now also tracks and RETURNS subtotal_before_cents
--    and member_discount_cents, so the confirmation screen can show the same
--    breakdown. Pricing/charging is otherwise unchanged from round-34a.
...
-- Update place_storefront_order to also return the pre-discount subtotal and the
-- member discount (everything else identical to round-34a).
...
begin
  ...
  loop
    ...
    v_deal_frac  := least(0.30, coalesce(v_pct, 0) / 100.0);
    v_total_frac := least(0.30, v_deal_frac + (v_tier_pct / 100.0));
    v_unit_deal := round(v_normal * (1 - v_deal_frac))::int;   -- deal price (no tier)
    v_unit      := round(v_normal * (1 - v_total_frac))::int;  -- charged price (deal + tier)
