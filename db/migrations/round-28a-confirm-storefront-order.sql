-- round-28a-confirm-storefront-order.sql
-- Confirms a DRAFT online order created by the storefront. Owner/admin only.
-- Mirrors the stock + commission logic of create_online_order, but operates on
-- the EXISTING draft + its existing sale_items instead of creating new ones,
-- and does NOT record payment or post to the ledger (that is Phase 4).
--
--   * Strict FIFO consume from the order's source warehouse (refuses oversell).
--   * Books seller commission (p_seller_id) + distributor commission
--     (the fulfillment warehouse's distributor), both 'pending'.
--   * Sets status='confirmed', confirmed_at, cogs, gross_profit.
--
-- Safe to re-run.

create or replace function public.confirm_storefront_order(
  p_sale_id uuid,
  p_seller_id uuid
)
returns jsonb
...
grant execute on function public.confirm_storefront_order(uuid, uuid) to authenticated;
