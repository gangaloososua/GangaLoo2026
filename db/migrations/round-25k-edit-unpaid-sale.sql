-- Round 25k - Edit Unpaid Sale (change products on a confirmed, unpaid sale)
--
-- Owner/admin/seller-gated. Replaces the full item set of a sale, but ONLY when
-- the sale is status='confirmed' AND paid_cents=0 (the safe case - no money has
-- moved, nothing posted to the ledger, no commissions paid out). Paid sales are
-- deliberately NOT editable here; they go through refund + re-ring instead.
--
-- One transaction:
--   2. REVERSE (clean replace): for each existing line, add its consumed qty
--      back to inventory_lots, then DELETE its stock_movements (clears the
--      fk_movements_sale_item FK), sale_lot_consumption, sale_commissions, and
--      sale_discount_applications; finally delete the sale_items. No audit
--      movements are kept - this is an unpaid correction, and lot quantities
--      stay exact. (Audit-trail preservation is reserved for the paid path.)
--   3. REPLAY: re-run confirm_pos_sale's exact per-item logic for the new item
--      set - FIFO lot consumption (same ORDER BY, same overshoot-first-lot
--      behavior => allow-but-warn on low stock, matching POS), COGS rollup,
--      seller commission (override -> product default -> 0, status 'pending').
--   4. Recompute subtotal/discount/cogs/gross_profit. total_cents is a GENERATED
--      column (subtotal - discount + tax + shipping) so it updates itself; we
--      must NOT write it. Sale stays 'confirmed' + unpaid.
--
-- Money in CENTS. The FK-before-delete ordering in step 2 was the fix for the
-- fk_movements_sale_item violation caught during the smoke test.
