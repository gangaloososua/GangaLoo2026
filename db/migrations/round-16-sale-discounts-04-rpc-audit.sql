-- Round 16.6 — Sale-discount audit persistence
--
-- Extends confirm_pos_sale and create_online_order to write rows into
-- sale_discount_applications so that every cent of discount applied at
-- the cart is attributable to either an auto rule, a manual line
-- override, or an order-level (sale-level) manual override.
--
-- Per-line audit logic (inside the items loop, after the sale_items
-- insert that produces v_sale_item_id):
--   * non-empty discount_breakdown in the item payload
--       -> one row per entry, is_manual=false, discount_rule_id=<rule_id>
--   * empty/missing discount_breakdown AND v_line_discount_cents > 0
--       -> one row, is_manual=true, discount_rule_id=NULL,
--          amount_cents=v_line_discount_cents
--   * else
--       -> no audit row for this line
--
-- Sale-level audit (immediately after the sales insert):
--   * v_sale_discount_cents > 0
--       -> one row keyed on sale_id (not sale_item_id), is_manual=true,
--          discount_rule_id=NULL, amount_cents=v_sale_discount_cents
--
-- The 'discount_breakdown' field in each item payload is OPTIONAL.
-- Callers that don't provide it produce no per-line audit rows, but
-- discount_cents on sale_items still reflects the right money. This
-- preserves backwards compat with any existing test fixtures and the
-- pre-16.5 form (none currently extant, but defensive).
--
-- Breakdown entry shape (snake_case, matching the SQL resolver):
--   { rule_id: uuid,
--     rule_kind: text (informational; not stored — kind lives on the rule),
--     percent: numeric or null,
--     amount_cents: int,
--     cap_hit: bool }
--
-- All other behavior (FIFO, commissions, totals, payments, stock
-- movements, status calculation) is unchanged. Diff vs prior version
-- is a few lines per function plus two new DECLARE entries.
