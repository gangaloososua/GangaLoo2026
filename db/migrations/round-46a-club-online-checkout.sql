-- round-46a-club-online-checkout.sql
-- Wire the Club membership discount into the ONLINE store: the checkout quote
-- (get_storefront_quote) and the actual charge (place_storefront_order), changed
-- together so they always agree.
--
-- Rule per line: discount fraction = LEAST(0.30, deal% + loyalty% + club%).
--   * deal%    = store_promotions (unchanged)
--   * loyalty% = get_customer_tier()       (earned by spend, unchanged)
--   * club%    = get_customer_club_pct()    (NEW: 15% flat for Club members)
-- Everything else (fees, shipping, items, invoice) is identical to round-34b.
--
-- SAFE: club% is 0 for everyone whose Club toggle is off, so prices are unchanged
-- for all but Club members (just the test customer today).

-- ─────────────────────────────────────────────────────────────────────────────
-- 1) get_storefront_quote — read-only checkout preview (now includes club%).
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  v_club_pct  := coalesce(get_customer_club_pct(v_cust_id), 0);
  ...
  loop
    ...
    -- deal alone (capped 30) vs deal + loyalty + club (capped 30).
    v_deal_frac  := least(0.30, coalesce(v_pct, 0) / 100.0);
    v_total_frac := least(0.30, v_deal_frac + ((v_tier_pct + v_club_pct) / 100.0));
    v_unit_deal := round(v_normal * (1 - v_deal_frac))::int;
    v_unit_comb := round(v_normal * (1 - v_total_frac))::int;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) place_storefront_order — the actual charge (now includes club%).
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  loop
    ...
    v_deal_frac  := least(0.30, coalesce(v_pct, 0) / 100.0);
    v_total_frac := least(0.30, v_deal_frac + ((v_tier_pct + v_club_pct) / 100.0));
    v_unit_deal := round(v_normal * (1 - v_deal_frac))::int;   -- deal price (no member discount)
    v_unit      := round(v_normal * (1 - v_total_frac))::int;  -- charged price (deal + loyalty + club)
