-- round-27e-storefront-order-fix.sql
-- Fixes round-27d: profiles.phone is UNIQUE across ALL roles (ux_profiles_phone),
-- so matching only role='customer' missed existing staff/owner numbers and the
-- insert collided. Now we match an existing profile by phone regardless of role
-- and reuse it; we only create a new profile when the phone is genuinely new.
-- Safe to re-run.
...
begin
  ...
  -- Match by phone across ALL roles (phone is globally unique); reuse if found.
