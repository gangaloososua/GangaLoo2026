-- round-33c-fix-generated-total.sql
-- Fix: round-33a's place_storefront_order tried to UPDATE sales.total_cents,
-- but total_cents is a GENERATED column (the DB computes it from
-- subtotal + shipping - discount + tax). Postgres rejects the write with:
--   428C9  column "total_cents" can only be updated to DEFAULT
--
-- Fix = don't touch total_cents at all; just set subtotal_cents (shipping_cents
-- was already set at insert time, so the generated total is correct on its own).
-- Everything else is identical to round-33a.
...
begin
  ...
  -- total_cents is a GENERATED column (subtotal + shipping - discount + tax),
  -- so we do NOT write it -- the DB computes it. We only set subtotal_cents;
  -- shipping_cents was already set at insert.
  update sales
     set subtotal_cents = v_subtotal
   where id = v_sale_id;

  -- Read the DB-computed total back for the response.
