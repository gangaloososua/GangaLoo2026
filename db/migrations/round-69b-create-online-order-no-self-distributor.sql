-- round-69b-create-online-order-no-self-distributor.sql
-- Same rule as 69a, applied to the staff manual-entry path (create_online_order):
-- the warehouse distributor does NOT earn the 5% distributor commission on an
-- order where they are themselves the SELLER or the CUSTOMER.
--
-- Rebuilt verbatim from the LIVE create_online_order body. Only change: after
-- resolving the warehouse distributor, drop it (no 5%) when it equals the
-- seller or the customer. v_customer_id and v_seller_id already exist.
...
BEGIN
  ...
  IF v_distributor_id IS NOT NULL
     AND (v_distributor_id = v_seller_id OR v_distributor_id = v_customer_id) THEN
