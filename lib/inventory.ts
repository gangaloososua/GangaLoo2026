...
export async function fetchStockOnHand(
  ...
  let lotQuery = supabase
    .from('inventory_lots')
    .select('product_id, warehouse_id, qty_remaining, products(name, sku), warehouses(name)')
  ...
  for (const lot of (data ?? []) as unknown as Array<{
    ...
    if (existing) {
      ...
      byKey.set(key, {
        ...
        productSku: lot.products?.sku ?? null,
...
export type StockMovementRow = {
  ...
  // Set on transfer_in / transfer_out movements -> /transfers/<transferId>.
  transferId: string | null
  // Set on purchase_in movements (resolved via the lot's PO item)
...
export async function fetchStockMovements(
  ...
  let query = supabase
    ...
    .select(
      'id, occurred_at, product_id, warehouse_id, kind, qty_delta, unit_cost_dop, adjustment_reason, transfer_id, products(name), warehouses(name), profiles(full_name), sale_items(sale_id, sales(id, invoice_number)), inventory_lots(purchase_order_items(purchase_order_id))',
    )
  ...
  return ((data ?? []) as unknown as Array<{
    ...
    inventory_lots: {
      purchase_order_items: { purchase_order_id: string | null } | null
    } | null
  }>).map((m) => ({
    id: m.id,
    ...
    purchaseOrderId:
      m.inventory_lots?.purchase_order_items?.purchase_order_id ?? null,
  }))
}
...
// Incoming (ordered, not yet arrived) units per product, summed across all
// open purchase orders. Uses the SAME open-PO statuses as the dashboard so
// the numbers reconcile. Returned as product_id -> qty for the stock view's
// "Incoming" column. Product-level (not split by warehouse).
export async function fetchIncomingByProduct(): Promise<Record<string, number>> {
  const supabase = await createClient()
  ...
  if (poErr) throw poErr
  const openPoIds = (openPos ?? []).map((p) => (p as { id: string }).id)
  if (openPoIds.length === 0) return {}

  const { data: items, error: itemsErr } = await supabase
    .from('purchase_order_items')
    .select('product_id, qty, purchase_order_id')
    .in('purchase_order_id', openPoIds)
  if (itemsErr) throw itemsErr
  ...
  for (const it of (items ?? []) as Array<{ product_id: string; qty: number }>) {
    const qty = Number(it.qty) || 0
    if (qty === 0) continue
    byProduct[it.product_id] = (byProduct[it.product_id] ?? 0) + qty
  }
  return byProduct
}
