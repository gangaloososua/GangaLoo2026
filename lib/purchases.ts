...
// ============================================================
// Round 41a - Partial supplier payments (read surface)
//
// READ ONLY. Lists the part-payments recorded against an order, with the
// paying account's name attached (same name-denormalisation pattern as the
// rest of this file). Amounts come back in pesos (cents / 100).
// ============================================================
...
export type SupplierPaymentRow = {
  ...
  category_id: string
  dop_amount: number               // pesos that left the account
  exchange_rate: number
  official_rate_at_payment: number | null
  usd_covered: number              // dollars of the order this payment covers
  paid_at: string
  created_at: string
}

export async function getSupplierPaymentsForOrder(
  orderId: string,
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, purchase_order_id, money_account_id, category_id, ' +
        'dop_amount_cents, exchange_rate, official_rate_at_payment, ' +
        'usd_covered, paid_at, created_at',
    )
    .eq('purchase_order_id', orderId)
  ...
  type RawSupplierPayment = {
    id: string
    purchase_order_id: string
    money_account_id: string
    category_id: string
    dop_amount_cents: number | string
    exchange_rate: number | string
    official_rate_at_payment: number | string | null
    usd_covered: number | string
    paid_at: string
    created_at: string
  }
  const raws = (data ?? []) as unknown as RawSupplierPayment[]
  if (raws.length === 0) return []

  const accountIds = Array.from(
    new Set(raws.map((r) => r.money_account_id).filter((x): x is string => !!x)),
  )
  const accountNameById = new Map<string, string>()
  ...
  return raws.map((r) => ({
    ...
    money_account_id: r.money_account_id,
    account_name: accountNameById.get(r.money_account_id) ?? null,
    category_id: r.category_id,
    dop_amount: Number(r.dop_amount_cents) / 100,
    exchange_rate: Number(r.exchange_rate),
    official_rate_at_payment:
      r.official_rate_at_payment == null ? null : Number(r.official_rate_at_payment),
    usd_covered: Number(r.usd_covered),
    paid_at: r.paid_at,
