...
const SWATCH: Record<string, string> = {
  'negro': '#1a1a1a',
  '#2 marron oscuro': '#3a2415',
  'marron oscuro': '#3a2415',
  '#4 marron': '#6b4423',
  'marron': '#6b4423',
}
function swatchFor(value: string): string | null {
  return SWATCH[value.trim().toLowerCase()] ?? null
}
const PAGE = 24
            ...
            <div className="flex-1 overflow-y-auto py-2">
              ...
              {categories.length > 0 && (
                <>
                  ...
                  <button type="button" onClick={() => jumpToCategory('all')} className="flex w-full items-center px-4 py-3 text-left text-[14px]" style={{ color: activeCat === 'all' ? NAVY : INK, fontWeight: activeCat === 'all' ? 600 : 400, background: activeCat === 'all' ? '#f0f4fb' : 'transparent' }}>
                    {ts(locale, 'shop.all')}
                  </button>
                  {categories.map((c) => (
                    <button key={c.id} type="button" onClick={() => jumpToCategory(c.id)} className="flex w-full items-center px-4 py-3 text-left text-[14px]" style={{ color: activeCat === c.id ? NAVY : INK, fontWeight: activeCat === c.id ? 600 : 400, background: activeCat === c.id ? '#f0f4fb' : 'transparent' }}>
                      {c.name}
                    </button>
                  ))}
                </>
              ...
              {attributes.map((attr) => (
                <div key={attr.id}>
                  <p className="mt-2 border-t px-4 pb-1.5 pt-4 text-[13px] font-semibold" style={{ color: NAVY, borderColor: '#eceef2' }}>{attr.name}</p>
                  {attr.values.map((v) => {
...
function AttributeDropdown({
  ...
  attr: { id: string; name: string; slug?: string; values: { id: string; value: string; slug?: string }[] }
  selected: Set<string>
  ...
  const count = attr.values.filter((v) => selected.has(v.id)).length
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex h-10 items-center gap-2 whitespace-nowrap rounded-lg px-5 text-[13px] transition active:scale-95"
        style={
          count > 0
            ? { background: '#fff', color: NAVY, fontWeight: 600, border: `1.5px solid ${NAVY}` }
            : { background: '#fff', color: INK, border: '1px solid #d7dde6' }
        }
      >
        <span>{attr.name}</span>
        {count > 0 && (
          <span className="flex min-w-[17px] items-center justify-center rounded-full px-1 text-[10px] font-semibold" style={{ height: 17, background: NAVY, color: '#fff' }}>
            {count}
          </span>
        )}
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1.5 min-w-[200px] rounded-xl bg-white py-2 shadow-lg" style={{ border: '1px solid #eceef2' }}>
          {count > 0 && (
            <button
              type="button"
              onClick={() => { onClear(); }}
              className="mb-1 w-full px-3 py-1.5 text-left text-[12px]"
              style={{ color: MUTED }}
            >
              {attr.name} limpiar
            </button>
          ...
          {attr.values.map((v) => {
            ...
            const sw = isColor ? swatchFor(v.value) : null
            return (
              <button
                key={v.id}
                type="button"
              ...
              >
                {isColor && (
                  <span
                    ...
                    style={{ width: 18, height: 18, background: sw ?? '#d7dde6', border: on ? `2px solid ${NAVY}` : '1px solid rgba(0,0,0,.15)' }}
                  />
                )}
                <span className="flex-1">{v.value}</span>
                {on && <Icon d="M5 12l4 4 10-10" size={14} />}
              </button>
            )
          })}
        </div>
...
function Chip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex h-10 items-center whitespace-nowrap rounded-lg px-5 text-[13px] transition active:scale-95" style={active ? { background: NAVY, color: '#fff', border: `1px solid ${NAVY}` } : { background: '#fff', color: INK, border: '1px solid #d7dde6' }}>
      {label}
    </button>
  )
}
