...
// Reports - Inventory valuation snapshot controls.
//
...
// Mirrors the Balance Sheet snapshot controls.

import { useEffect, useRef, useTransition } from 'react'
import { useRouter } from 'next/navigation'

import { Button } from '@/components/ui/button'
import { saveInventorySnapshotAction } from './actions'
...
export function SnapshotControls({
  ...
  useEffect(() => {
    ...
    if (!haveThisMonth) {
      startTransition(async () => {
        await saveInventorySnapshotAction()
        router.refresh()
      })
    }
  ...
  function onSelect(value: string) {
    if (value === 'live') router.push('/reports/inventory')
    else router.push(`/reports/inventory?month=${value}`)
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <select
        ...
        aria-label="Inventory valuation date"
      >
