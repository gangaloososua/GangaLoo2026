...
import { SupplierPaymentsList } from './supplier-payments-list'
...
export default async function PurchaseDetailPage({
  ...
  const [items, lotTrail, transport, moneyAccounts, categories, supplierPayments] = await Promise.all([
    getPurchaseOrderItems(order.id),
    getLotTrailForOrder(order.id),
    getTransportSummaryForOrder(order.id),
    listMoneyAccounts(),
    listAccountCategories(),
    getSupplierPaymentsForOrder(order.id),
  ])
              ...
              {paid ? (
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="tabular-nums">${formatNumber(usdCovered)}</span>
                  </div>
                  <div className="flex justify-between font-semibold">
                    <span>Still open</span>
                    <span className="tabular-nums">${formatNumber(usdRemaining)}</span>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Of ${formatNumber(usdTotal)} total. Use the &ldquo;Pay supplier&rdquo;
                    button to add another payment.
                  </div>
