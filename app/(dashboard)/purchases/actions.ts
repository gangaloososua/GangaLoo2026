...
export async function addSupplierPayment(
  ...
  if (!input.orderId) return { ok: false, error: 'Order id is required.' }
  if (!input.supplierPaymentAccountId)
    return { ok: false, error: 'Pick the account this part was paid from.' }
  if (!Number.isFinite(input.dopAmount) || input.dopAmount <= 0)
    return { ok: false, error: 'Payment amount (DOP) must be greater than zero.' }
  if (!Number.isFinite(input.exchangeRate) || input.exchangeRate <= 0)
    return { ok: false, error: 'Exchange rate must be greater than zero.' }
  if (!Number.isFinite(input.officialRateAtPayment) || input.officialRateAtPayment <= 0)
    return { ok: false, error: 'Official rate must be greater than zero.' }
...
export async function removeSupplierPayment(
  ...
  const supabase = await createClient()
  const { error } = await supabase.rpc('remove_supplier_payment', {
    p_payment_id: paymentId,
  })
  if (error) return { ok: false, error: error.message }

  if (orderId) revalidatePath(`/purchases/${orderId}`)
  revalidatePath('/purchases')
  return { ok: true }
}
